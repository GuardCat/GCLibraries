...
def process_video_with_debug(video_path, output_path, conf_threshold=0.6):  # Увеличен порог уверенности
    ...
    while cap.isOpened():
        ...
        results = model.track(
            ...
            persist=True,
            verbose=False,
            conf=conf_threshold,  # Установлен высокий порог уверенности
        ...
        if results[0].boxes.id is not None:
            ...
            for box, conf, track_id, cls_id in zip(boxes, confs, track_ids, class_ids):
                ...
                # Регистрация новых объектов только при высокой уверенности
                if conf >= conf_threshold and track_id not in vehicle_registry[class_name]:
